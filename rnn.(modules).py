        ...
        if setting == 5:
            ...
            for layer in range(num_layers):
                for direction in range(num_directions):
                    ...
                    w_ih = Parameter(torch.Tensor(gate_size, layer_input_size))
                    w_hh = Parameter(torch.Tensor(gate_size, hidden_size))
                    w_2 = Parameter(torch.Tensor(gate_2))
                    b_ih = Parameter(torch.Tensor(gate_size))
                    b_hh = Parameter(torch.Tensor(gate_size))
                    layer_params = (w_ih, w_hh, w_2, b_ih, b_hh)
...
###################################################################################################################################################
    if setting == 3: 
        def __init__(self, input_size, hidden_size, bias=True):
            ...
            self.hidden_size = hidden_size
            self.bias = bias
            self.weight_ih = Parameter(torch.Tensor(8 * hidden_size, input_size))
            self.weight_hh = Parameter(torch.Tensor(8 * hidden_size, hidden_size))
            if bias:
                self.bias_ih = Parameter(torch.Tensor(8 * hidden_size))
                self.bias_hh = Parameter(torch.Tensor(8 * hidden_size))
...
###################################################################################################################################################
    if setting == 4:
        def __init__(self, input_size, hidden_size, bias=True):
            ...
            self.hidden_size = hidden_size
            self.bias = bias
            self.weight_ih = Parameter(torch.Tensor(9 * hidden_size, input_size))
            self.weight_hh = Parameter(torch.Tensor(9 * hidden_size, hidden_size))
            if bias:
                self.bias_ih = Parameter(torch.Tensor(9 * hidden_size))
                self.bias_hh = Parameter(torch.Tensor(9 * hidden_size))
            else:
                self.register_parameter('bias_ih', None)
                self.register_parameter('bias_hh', None)
            self.reset_parameters()
    ...
    if setting == 9:
        if preset == False:
            def __init__(self, input_size, hidden_size, bias=True):
                ...
                if bias:
                    self.bias_ih = Parameter(torch.Tensor(6 * hidden_size))
                    self.bias_hh = Parameter(torch.Tensor(6 * hidden_size))
                else:
                    self.register_parameter('bias_ih', None)
                    self.register_parameter('bias_hh', None)
                self.reset_parameters()
